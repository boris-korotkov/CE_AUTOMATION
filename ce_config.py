...
def load_hotkey_config():
    """Loads hotkey settings from the [Hotkeys] section."""
    hotkeys = {
        'pause_resume': 'ctrl+p', # Default values
...
def load_instances():
    ...
    for section in config.sections():
        ...
        details = {
            ...
            "game_load_check_region": config.get(section, "game_load_check_region", fallback="").strip(),
            "game_load_check_text": config.get(section, "game_load_check_text", fallback="").strip(),
        }
...
def load_run_order():
    ...
    if config.has_section("RunOrder"):
        ...
        if start_instance:
            start_instance = start_instance.strip()
